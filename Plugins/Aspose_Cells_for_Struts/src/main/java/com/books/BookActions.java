...
public class BookActions extends DispatchAction {
	...
	/**
	 * Returns excel file that can then be downloaded locally. 
	 * @see         AsposeAPIHelper
	 */
	public ActionForward ExportToExcel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		System.out.println("Aspose export excel sheet");
		...
		response.setHeader("Content-Disposition",
				"attachment;filename=AsposeExportBooksList.xlsx");
		for (Map book : books) {
			try {
				AsposeAPIHelper.createAsposeExcelSheet(response.getOutputStream(),
