...
public class AddImageXlsx4j
{
	public static void main(String[] args) throws Exception
	{
		...
		String outputfilepath = dataPath + "AddImage-Xlsx4j.xlsx";
		String imagefilePath = dataPath + "greentick.png";

		SpreadsheetMLPackage pkg = SpreadsheetMLPackage.createPackage();
		WorksheetPart worksheet = pkg.createWorksheetPart(new PartName(
				"/xl/worksheets/sheet1.xml"), "Sheet1", 1);
		...
		org.xlsx4j.sml.CTDrawing drawing = org.xlsx4j.jaxb.Context
				.getsmlObjectFactory().createCTDrawing();
		...
		BinaryPartAbstractImage imagePart = BinaryPartAbstractImage
				.createImagePart(pkg, drawingPart,
		...
		drawingPart
				.setJaxbElement(buildDrawingPartContentFromXmlString(imageRelID));
	 ...
	 * http://webapp.docx4java.org/OnlineDemo/PartsList.html "Method 1"
	 */
	public static CTDrawing buildDrawingPartContentUsingCode(String imageRelID)
	{
		...
		CTDrawing drawing = dmlspreadsheetdrawingObjectFactory
				.createCTDrawing();
		...
		CTTwoCellAnchor twocellanchor = dmlspreadsheetdrawingObjectFactory
				.createCTTwoCellAnchor();
		drawing.getEGAnchor().add(twocellanchor);
		// Create object for clientData
		CTAnchorClientData anchorclientdata = dmlspreadsheetdrawingObjectFactory
				.createCTAnchorClientData();
		twocellanchor.setClientData(anchorclientdata);
		// Create object for pic
		CTPicture picture = dmlspreadsheetdrawingObjectFactory
				.createCTPicture();
		twocellanchor.setPic(picture);
		org.docx4j.dml.ObjectFactory dmlObjectFactory = new org.docx4j.dml.ObjectFactory();
		// Create object for blipFill
		CTBlipFillProperties blipfillproperties = dmlObjectFactory
		...
		CTOfficeArtExtensionList officeartextensionlist = dmlObjectFactory
				.createCTOfficeArtExtensionList();
		blip.setExtLst(officeartextensionlist);
		// Create object for ext
		CTOfficeArtExtension officeartextension = dmlObjectFactory
		...
		CTStretchInfoProperties stretchinfoproperties = dmlObjectFactory
		...
		point2d.setX(0);
		// Create object for ext
		CTPositiveSize2D positivesize2d = dmlObjectFactory
		...
		CTPresetGeometry2D presetgeometry2d = dmlObjectFactory
				.createCTPresetGeometry2D();
		shapeproperties.setPrstGeom(presetgeometry2d);
		// Create object for avLst
		CTGeomGuideList geomguidelist = dmlObjectFactory
		...
		CTPictureNonVisual picturenonvisual = dmlspreadsheetdrawingObjectFactory
				.createCTPictureNonVisual();
		picture.setNvPicPr(picturenonvisual);
		// Create object for cNvPr
		CTNonVisualDrawingProps nonvisualdrawingprops = dmlObjectFactory
		...
		CTNonVisualPictureProperties nonvisualpictureproperties = dmlObjectFactory
				.createCTNonVisualPictureProperties();
		picturenonvisual.setCNvPicPr(nonvisualpictureproperties);
		// Create object for picLocks
		CTPictureLocking picturelocking = dmlObjectFactory
		...
		marker2.setRowOff(0);
		twocellanchor
				.setEditAs(org.docx4j.dml.spreadsheetdrawing.STEditAs.ONE_CELL);
	 ...
	 */
	public static CTDrawing buildDrawingPartContentFromXmlString(
			String imageRelID) throws JAXBException
	{
